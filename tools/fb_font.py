"""MicroPython framebuf built-in 8x8 font (petme128), MIT, Damien George.
Column-major: 8 bytes/glyph from ASCII 32; byte=column, bit i = row i (top=0).
Embedded so the desktop preview renders the OLED pixel-for-pixel.
"""

FONT8 = bytes([
    ,
])
